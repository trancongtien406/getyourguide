...
export default function SupplierNewTourPage() {
  ...
  const [tourOptions, setTourOptions] = useState<TourOptionDraft[]>([]);
  ...
  // ── Load reference data + supplier info ─────────────────────────
  useEffect(() => {
    async function loadData() {
      try {
        const [catRes, tagRes, cityRes, langRes, supplierRes] = await Promise.all([
        ...
        if (supplierRes.length === 1) {
          setSupplierId(supplierRes[0].id);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/profile/supplier/tours/${tour.id}/edit`);
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          ...
          <div className="space-y-4">
            ...
            {suppliers.length === 1 && (
              <div className="rounded-lg bg-gray-50 px-4 py-2.5 text-sm text-gray-700 dark:bg-gray-800 dark:text-gray-300">
                <span className="font-medium">{t('labelSupplier')}:</span>{' '}
                {suppliers[0].displayName}
              </div>
            )}
            <Input
              label={t('labelTitle')}
