...
export class UserManagementService {
  ...
  async listUsers(actor: JwtPayload, query: ListUsersDto) {
    ...
    if (actorRoleLevel === 'SUPPLIER_ADMIN') {
      supplierStaffUserIds =
        await this.getSupplierStaffIdsManageableBySupplierAdmin(actor.sub);
  ...
  async getUserById(actor: JwtPayload, userId: string) {
    ...
    if (actorRoleLevel === 'SUPPLIER_ADMIN') {
      const supplierStaffIds =
        await this.getSupplierStaffIdsManageableBySupplierAdmin(actor.sub);
      if (!supplierStaffIds.includes(userId)) {
        throw new ForbiddenException(
          'Supplier admin can only view supplier staff',
        );
      }
    }
  ...
  async createUser(actor: JwtPayload, dto: CreateUserDto) {
    ...
    const status =
      dto.status ??
      (actorRoleLevel === 'SUPPLIER_ADMIN' ? 'ACTIVE' : 'PENDING');
    ...
    const createdUser = await this.prisma.$transaction(async (tx) => {
      ...
      const supplierRoles = roles.filter(
        (role) => role === 'SUPPLIER_ADMIN' || role === 'SUPPLIER_STAFF',
      );
  ...
  async updateUserById(actor: JwtPayload, userId: string, dto: UpdateUserDto) {
    ...
    if (actor.sub === userId)
      throw new BadRequestException('Use /auth/me to update your own profile');
    if (actorRoleLevel === 'OPERATOR' && targetRoles.has('ADMIN'))
      throw new ForbiddenException('Operator cannot update admin user');
    if (
      actorRoleLevel === 'SUPPLIER_ADMIN' &&
      !targetRoles.has('SUPPLIER_STAFF')
    )
      throw new ForbiddenException(
        'Supplier admin can only update supplier staff',
      );

    const allowedFields =
      this.getAllowedUpdateFieldsByRoleLevel(actorRoleLevel);
    ...
    if (
      actorRoleLevel === 'SUPPLIER_ADMIN' &&
      dto.status &&
      !['ACTIVE', 'SUSPENDED'].includes(dto.status)
    ) {
      throw new ForbiddenException(
        'Supplier admin can only set ACTIVE or SUSPENDED status',
      );
    }
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (dto.roles && dto.roles.length > 0) {
        if (actorRoleLevel !== 'ADMIN')
          throw new ForbiddenException('Only admin can change user roles');
        if (dto.roles.includes('ADMIN') && actorRoleLevel !== 'ADMIN')
  ...
  async lockUserById(
    ...
    return this.applyUserLifecycleAction(actor, userId, dto, {
      ...
      nextDeletedAt: null,
      allowedCurrentStatuses: ['ACTIVE', 'PENDING'],
      requireDeletedAt: false,
      action: 'USER_LOCKED',
      revokeSessions: true,
    });
  }
  ...
  async softDeleteUserById(
    ...
    return this.applyUserLifecycleAction(actor, userId, dto, {
      ...
      allowedCurrentStatuses: ['PENDING', 'ACTIVE', 'SUSPENDED'],
      requireDeletedAt: false,
  ...
  async restoreUserById(
    ...
    return this.applyUserLifecycleAction(actor, userId, dto, {
      nextStatus: 'ACTIVE',
      nextDeletedAt: null,
      allowedCurrentStatuses: ['DELETED'],
      requireDeletedAt: true,
  ...
  async unlockUsersBulk(actor: JwtPayload, dto: BulkUserLifecycleActionDto) {
    return this.applyBulkLifecycleAction(actor, dto, {
      ...
      action: 'USER_UNLOCKED',
      revokeSessions: false,
      bulkAction: 'USER_BULK_UNLOCKED',
    });
  }

  async softDeleteUsersBulk(
    actor: JwtPayload,
    ...
    return this.applyBulkLifecycleAction(actor, dto, {
      ...
      action: 'USER_SOFT_DELETED',
      revokeSessions: true,
      bulkAction: 'USER_BULK_SOFT_DELETED',
    });
  }
  ...
  async resetUserPasswordByAdmin(
    ...
    if (actor.sub === userId)
      throw new BadRequestException(
        'Use /auth/change-password for your own account',
      );
    ...
    return {
      message: 'User password has been reset and active sessions were revoked',
    };
  }
  ...
  private async applyUserLifecycleAction(
    ...
    options: {
      ...
      nextDeletedAt: Date | null;
      allowedCurrentStatuses: UserStatus[];
    ...
    if (!options.allowedCurrentStatuses.includes(targetUser.status)) {
      throw new BadRequestException(
        `Invalid current status for this action: ${targetUser.status}`,
      );
    }
    if (options.requireDeletedAt && !targetUser.deletedAt)
      throw new BadRequestException('User is not deleted');
    if (
      !options.requireDeletedAt &&
    ...
    const updatedUser = await this.prisma.$transaction(async (tx) => {
      await tx.user.update({
        where: { id: userId },
        data: { status: options.nextStatus, deletedAt: options.nextDeletedAt },
      });
  ...
  private async applyBulkLifecycleAction(
    ...
    for (const userId of dto.userIds) {
      try {
        ...
        await this.assertCanManageTargetUser(
          actorRoleLevel,
          actor.sub,
          targetUser,
        );
        ...
        await this.prisma.$transaction(async (tx) => {
          await tx.user.update({
            ...
            data: {
              ...
              deletedAt: options.nextDeletedAt,
            },
          });
          if (options.revokeSessions) {
            await tx.userSession.updateMany({
        ...
        const message =
          error instanceof Error && error.message
            ? error.message
            : 'Unexpected error while processing user';
  ...
  private async assertCanManageTargetUser(
    ...
    if (actorRoleLevel === 'SUPPLIER_ADMIN') {
      if (!targetRoles.has('SUPPLIER_STAFF'))
        throw new ForbiddenException(
          'Supplier admin can only manage supplier staff',
        );
      const supplierStaffIds =
        await this.getSupplierStaffIdsManageableBySupplierAdmin(actorUserId);
      if (!supplierStaffIds.includes(targetUser.id))
        throw new ForbiddenException('Supplier admin cannot manage this user');
    }
  }

  private resolveActorRoleForAudit(
  ...
  private async getSupplierStaffIdsManageableBySupplierAdmin(userId: string) {
    const managedSupplierIds =
      await this.getManagedSupplierIdsBySupplierAdmin(userId);
  ...
  private resolveUsersOrderBy(
    ...
    switch (query.sortBy) {
      ...
      case 'lastloginat':
        return [{ lastLoginAt: sortOrder }, { createdAt: 'desc' }];
      case 'updatedat':
  ...
  private resolveCreateRoles(
    actorRoleLevel: 'ADMIN' | 'OPERATOR' | 'SUPPLIER_ADMIN' | 'NONE',
    requestedRoles?: UserRole[],
  ) {
    const deduped = requestedRoles ? [...new Set(requestedRoles)] : [];
    if (deduped.length === 0) {
      return actorRoleLevel === 'SUPPLIER_ADMIN'
        ? (['SUPPLIER_STAFF'] as UserRole[])
        : (['CUSTOMER'] as UserRole[]);
    }
    const allowedRolesByLevel: Record<
      'ADMIN' | 'OPERATOR' | 'SUPPLIER_ADMIN' | 'NONE',
      UserRole[]
    > = {
      ADMIN: [
        'CUSTOMER',
    ...
    if (deduped.some((role) => !allowed.has(role))) {
      throw new ForbiddenException(
        'Some roles are not allowed for your account',
      );
    }
    return deduped;
  }
  ...
  private async resolveSupplierIdsForCreate(
    ...
    const hasSupplierRole = roles.some(
      (role) => role === 'SUPPLIER_ADMIN' || role === 'SUPPLIER_STAFF',
    );
    if (!hasSupplierRole) return [];

    const dedupedRequested = requestedSupplierIds
      ? [...new Set(requestedSupplierIds.map((id) => id.trim()))].filter(
          Boolean,
        )
      : [];

    if (actorRoleLevel === 'SUPPLIER_ADMIN') {
      const manageableSupplierIds =
        await this.getManagedSupplierIdsBySupplierAdmin(actorUserId);
      if (!manageableSupplierIds.length)
        throw new ForbiddenException(
          'Supplier admin has no manageable suppliers',
        );
      if (!dedupedRequested.length) return manageableSupplierIds;
      const manageableSet = new Set(manageableSupplierIds);
      const allowedSupplierIds = dedupedRequested.filter((id) =>
        manageableSet.has(id),
      );
      if (!allowedSupplierIds.length)
        throw new ForbiddenException(
          'No supplierIds are manageable by your account',
        );
    ...
    if (!dedupedRequested.length)
      throw new BadRequestException(
        'supplierIds is required when assigning supplier roles',
      );
    return dedupedRequested;
  }

  private getAllowedUpdateFieldsByRoleLevel(
    roleLevel: 'ADMIN' | 'OPERATOR' | 'SUPPLIER_ADMIN' | 'NONE',
  ) {
    if (roleLevel === 'ADMIN')
      return new Set(['firstName', 'lastName', 'phoneE164', 'status', 'roles']);
    if (roleLevel === 'OPERATOR')
      return new Set(['firstName', 'lastName', 'phoneE164', 'status']);
    if (roleLevel === 'SUPPLIER_ADMIN')
      return new Set(['firstName', 'lastName', 'phoneE164', 'status']);
    return new Set<string>();
  }

  private hasRestrictedUpdateInput(
    dto: UpdateUserDto,
    ...
    return checks.some(
      ([field, value]) => value !== undefined && !allowedFields.has(field),
    );
  }
}
