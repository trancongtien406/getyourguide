...
export default function EditTourPage({
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setIsFeatured(tourRes.isFeatured ?? false);
        setBadgeText(tourRes.badgeText || '');
        ...
        // Categories & tags (backend returns relation objects)
        ...
        const tabCodes = translations.map((t) => t.languageCode);
        setTranslationTabs(tabCodes);

        const forms: Record<string, TranslationFormData> = {};
        for (const t of translations) {
          forms[t.languageCode] = translationToForm(t);
        }
  ...
  // ── Load departure summary for saved options ─────────────────────
  ...
  useEffect(() => {
    for (const opt of tourOptions) {
      if (opt.id && !depSummary[opt.id]) {
        loadDepSummary(opt.id);
      }
    }
  }, [tourOptions, loadDepSummary, depSummary]);

  const handleBulkGenerate = useCallback(async (optionId: string) => {
  ...
  }, [durationMinutes, loadDepSummary, showMessage, t]);
  ...
  const removeLanguageTab = async (langCode: string) => {
    ...
    if (
      !confirm(t('confirmDeleteTranslation', { name: langName }))
    )
      return;
    ...
    try {
      ...
      // Translation may not exist in DB yet (just a tab), ignore
    }
        ...
        {activeTab === 'default' && (
          <>
            ...
            <Card>
              ...
              />
              {tourOptions.filter(o => o.id).length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400 px-6 pb-4">{t('saveOptionsFirst')}</p>
              ) : (
                <div className="space-y-4 px-6 pb-6">
                  {tourOptions.filter(o => o.id).map(opt => {
                    const summary = depSummary[opt.id!];
                    const isLoading = depLoading[opt.id!];
                    const today = new Date().toISOString().split('T')[0];
                    const in14 = new Date(Date.now() + 14 * 86400000).toISOString().split('T')[0];
                    return (
                      <div key={opt.id} className="rounded-lg border border-gray-200 dark:border-gray-700 p-4 space-y-3">
                        <div className="flex items-center justify-between">
                        ...
                        <div className="flex flex-wrap gap-2 items-end">
                          <div>
                            ...
                            <input type="date" id={`dep-start-${opt.id}`} defaultValue={today}
                          ...
                          <div>
                            ...
                            <input type="date" id={`dep-end-${opt.id}`} defaultValue={in14}
                              className="rounded-md border border-gray-300 px-3 py-1.5 text-sm dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200" />
                          </div>
                          <div>
                            <label className="block text-xs text-gray-500 mb-1">{t('departureTimes')}</label>
                            <input type="text" id={`dep-times-${opt.id}`} defaultValue="09:00, 14:00" placeholder="09:00, 14:00"
                              className="w-36 rounded-md border border-gray-300 px-3 py-1.5 text-sm dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200" />
                          </div>
                          <div>
                            <label className="block text-xs text-gray-500 mb-1">{t('capacity')}</label>
                            <input type="number" id={`dep-cap-${opt.id}`} defaultValue={maxGroupSize || '20'} min={1}
                              className="w-20 rounded-md border border-gray-300 px-3 py-1.5 text-sm dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200" />
                          </div>
                          <Button type="button" variant="outline" size="sm" onClick={() => handleBulkGenerate(opt.id!)}>
                            {t('generate')}
                          </Button>
                        </div>
                      </div>
            ...
            <Card>
              ...
              <div className="space-y-4">
                ...
                <Input
                  label={t('labelBadgeText')}
                  value={badgeText}
                  onChange={(e) => setBadgeText(e.target.value)}
                  placeholder={t('placeholderBadgeText')}
                />
              </div>
            </Card>
