...
export class BookingsController {
  ...
  @Post()
  createBooking(@CurrentUser() actor: JwtPayload, @Body() dto: CreateBookingDto) {
    return this.bookingsService.createBooking(actor, dto);
  }

  @UseGuards(JwtAuthGuard)
  @Get('me')
  listMyBookings(@CurrentUser() actor: JwtPayload, @Query() query: ListMyBookingsDto) {
    return this.bookingsService.listMyBookings(actor, query);
  }

  @UseGuards(JwtAuthGuard)
  @Get('supplier')
  listSupplierBookings(@CurrentUser() actor: JwtPayload, @Query() query: ListSupplierBookingsDto) {
    return this.bookingsService.listSupplierBookings(actor, query);
  }

  @UseGuards(JwtAuthGuard)
  @Get('admin')
  listAllBookings(@CurrentUser() actor: JwtPayload, @Query() query: ListAllBookingsDto) {
    return this.bookingsService.listAllBookings(actor, query);
  }

  @Get('guest/:id')
  getGuestBookingById(@Param('id') id: string) {
    return this.bookingsService.getGuestBookingById(id);
  }
